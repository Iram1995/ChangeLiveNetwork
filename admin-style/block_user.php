...
<?php   
    ...
    $annUser="";
    $annUser=isset($_GET['blockusername'])?$_GET['blockusername']:'na';

    $sqlAnn=mysqli_query($con,"SELECT * FROM mlmmembers WHERE username='$annUser'");
    $count=mysqli_num_rows($sqlAnn);
    
    if($adminBlockUser = mysqli_query($con,"UPDATE mlmmembers SET status='0' WHERE username='$annUser'")){
        $adBlockMsg = '<p class="alert alert-success">You have blocked <b>'.$annUser.'</b></p>';
    }
...
<!DOCTYPE html>
<head>
<title>Change Lives Network : Block Member</title>
